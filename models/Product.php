...
class Product{
    ...
    static public function getProductById($data){
        ...
        try{
            ...
            $stmt->execute(array(":id" => $id));
            $product = $stmt->fetch(PDO::FETCH_OBJ); // 7itach ghir produit wahd
            return $product;
        }catch(PDOException $ex){
            echo "erreur " . $ex->getMessage();
        }
    ...
    static public function addProduct($data){
        try{
            ...
            ,product_description,product_quantity,product_image,product_price,old_price
            ,short_desc,product_category_id)
            VALUES 
            (:product_title,:product_description,:product_quantity,
            :product_image,:product_price,:old_price,:short_desc,:product_category_id)');
